...
/* 注意：之所以遇到0（R）和遇到2（B）时对mid的处理不同，是因为：
    遇到 0 时：交换后mid位置的元素一定是合法的（0 或 1），因此可以直接移动mid。
    遇到 2 时：交换后mid位置的元素可能是未处理的 0、1 或 2，因此需要保持mid不变，在下一轮循环中重新检查。
*/
